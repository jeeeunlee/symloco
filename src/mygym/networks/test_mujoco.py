...
if __name__ == "__main__":
    ...
    # Set initial qpos using the position from the XML file
    torso_body_index = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "torso")
    initial_torso_pos = model.body_pos[torso_body_index]

    init_qpos = data.qpos.ravel().copy()
    init_qvel = data.qvel.ravel().copy()

    # Update the initial base position from XML values
    init_qpos[0:3] = initial_torso_pos
    ...
    for t in range(1000):
        ...
        # Update qpos values to some values
        data.qpos[0:3] = initial_torso_pos
        data.qpos[3:6] = [0.1, 0.1, 0.1]
        data.qpos[6:9] = [-0.1, -0.1, -0.1]
