...
class TargetVelcocityGenerator(ABC):
...
class SinusoidalVelcocityGenerator(TargetVelcocityGenerator):
    def __init__(self, 
                 dim,
                 freq = DEFAULT_VELOCITY_PROFILE["freq"], 
                 mag = DEFAULT_VELOCITY_PROFILE["mag"]):
        super().__init__(dim)
        assert len(freq) == dim and len(mag)==dim, "Frequency and magnitude lists must match the dimension."
        self.freq = freq
        self.mag = mag
...
class BiasedSinusoidalVelcocityGenerator(TargetVelcocityGenerator):
    def __init__(self, 
                 dim,
                 freq = DEFAULT_VELOCITY_PROFILE["freq"], 
                 mag = DEFAULT_VELOCITY_PROFILE["mag"],
